...
internal class ExportTokensCommand(val accessTokenFacade: AccessTokenFacade) : ReposiliteCommand {

    @Parameters(index = "0", paramLabel = "<file>", description = ["Path to destination JSON file"])
    lateinit var name: String

    override fun execute(context: CommandContext) {
        accessTokenFacade.exportToFile(name).also { (path, tokens) ->
            context.append("${tokens.size} token(s) found.")
            context.append("All tokens have been exported to ${path.absolute()} in JSON format.")
        }
    }
...
internal class ImportTokensCommand(val accessTokenFacade: AccessTokenFacade) : ReposiliteCommand {

    @Parameters(index = "0", paramLabel = "<file>", description = ["Path to JSON file with access tokens"])
    lateinit var name: String

    override fun execute(context: CommandContext) {
        accessTokenFacade.importFromFile(name)
            .onError {
