...
import com.reposilite.packages.maven.api.Checksum
...
import com.reposilite.packages.maven.api.PomDetails
...
internal class MavenFacadeTest : MavenSpecification() {
    ...
    inner class Access {
        ...
        @ParameterizedTest
        @EnumSource(value = MavenRepositoryVisibility::class, names = [ "PUBLIC", "HIDDEN" ])
        fun `should find requested details without credentials in public and hidden repositories`(visibility: MavenRepositoryVisibility) {
        ...
        @ParameterizedTest
        @EnumSource(value = MavenRepositoryVisibility::class, names = [ "HIDDEN", "PRIVATE" ])
        fun `should restrict directory indexing in hidden and private repositories `(visibility: MavenRepositoryVisibility) {
            // given: a repository with a file
            val fileSpec = addFileToRepository(FileSpec(visibility.name, "/gav/file.pom", "content"))
    ...
    inner class Modifications {
        ...
        @ParameterizedTest
        @EnumSource(MavenRepositoryVisibility::class)
        fun `should deploy file under the given path`(visibility: MavenRepositoryVisibility) {
        ...
        fun `should deploy file and generate checksums`() {
            ...
            val deployResult = mavenFacade.deployFile(
                DeployRequest(
                    mavenRepository = fileSpec.repository(),
                    gav = fileSpec.gav(),
                    by = by,
