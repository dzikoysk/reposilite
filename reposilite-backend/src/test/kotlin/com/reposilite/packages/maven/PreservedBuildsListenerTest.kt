...
import com.reposilite.packages.maven.api.Snapshot
import com.reposilite.packages.maven.api.SnapshotVersion
...
internal class PreservedBuildsListenerTest : MavenSpecification() {
    ...
    fun `should remove deprecated snapshot versions`() {
        ...
        mavenFacade.saveMetadata(
            SaveMetadataRequest(
                mavenRepository = mavenFacade.getRepository(repositoryName)!!,
                gav = versionId,
                metadata = Metadata(
        ...
        assertCollectionsEquals(
            ...
            assertOk(repository.storageProvider.getFiles(versionId))
        )
    }
