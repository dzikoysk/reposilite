...
const reportUrl = (failure) => {
  const logs = failure.length > MAX_LOG_LENGTH
    ? failure.slice(0, MAX_LOG_LENGTH) + '\n… (truncated)'
    : failure

  const params = new URLSearchParams({
    template: 'bug-report.yml',
    version: '3.x',
    'what-happened': `Failure automatically captured by the Reposilite dashboard (v${props.version}). Please describe what you were doing when it occurred.`,
    logs
  })
  ...
  <div>
    <template v-for="(entry, index) in parsedFailures" :key="entry.key">
      ...
      >
        ...
        <div class="fact">
          <a :href="reportUrl(entry.failure)" target="_blank" class="report-link" title="Report on GitHub" @click.stop>
            Report on GitHub
            <LinkIcon />
          </a>
        </div>
      ...
      <div v-if="openIndex === index" class="row-detail open">
        <div class="trace">
          <button class="copy" title="Copy trace" @click="copyTrace(entry.failure, index)">
            <span v-if="copiedIndex === index">Copied</span>
            <CopiedIcon v-if="copiedIndex === index" />
            <CopyIcon v-else />
          </button>
          <pre>{{ entry.failure }}</pre>
        </div>
      </div>
    </template>
  </div>
...
.ftype { @apply font-mono text-sm font-semibold text-red-600 dark:text-red-400 flex-none <sm:text-xs; }
