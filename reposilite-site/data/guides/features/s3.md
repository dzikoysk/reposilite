...
`Note` - **Shared Bucket must be enabled on every repository that shares a bucket.** If two repositories
target the same bucket and one of them does not have it enabled, that repository would collide at the
bucket root, so Reposilite refuses to load it and logs an error on startup.
